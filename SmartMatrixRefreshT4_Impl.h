// todo: configurable number of shifters
...
FLASHMEM void SmartMatrixRefreshT4<refreshDepth, matrixWidth, matrixHeight, panelType, optionFlags>::hardwareSetup() {
  ...
  // Configure three FlexIO Shifters:
  ...
  triggerSelect = FLEXIO_TIMCTL_TRGSEL(1 + 4 * (RGBDATA_SHIFTERS - 1)); // Trigger select Shifter 1 status flag
  ...
  *timerRegisterLatch = dutyCycleLatch;
  ...
  // dmaClockOutData transfers 32-bit words from the rowDataBuffer to the 32-bit FlexIO shifter registers. For improved speed, we configure it
  ...
  // DMA is fast enough at 600 MHz to refill the shifter registers before they underrun with FlexIO running at FLEXIO_CLOCK_DIVIDER>=20. This ensures
  // that the shifters do not underrun until dmaClockOutData completes at the end of the row (allowing the Shifter 2 address data to output). */
  ...
  destinationAddressModulo = 3; // keep the destination address fixed except for the 3 least significant bits, effectively making an 8 byte circular buffer
  ...
  dmaClockOutData.TCD->ATTR_DST = DMA_TCD_ATTR_DMOD(destinationAddressModulo) | DMA_TCD_ATTR_SIZE_32BIT;
  dmaClockOutData.TCD->DLASTSGA = destinationAddressLastOffset;
  dmaClockOutData.TCD->NBYTES = minorLoopBytes;
...
FASTRUN void SmartMatrixRefreshT4<refreshDepth, matrixWidth, matrixHeight, panelType, optionFlags>::setRowAddress(unsigned int row) {
  ...
  flexIO->SHIFTBUF[2] = addressData; // load third FlexIO shift register with address data
}
