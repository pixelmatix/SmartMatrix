/*
 * SpectrumAnalyzer I2S Example
 *
 * This version uses I2S Digital Input for Audio with the Teensy 4, either from the Teensy Audio Shield for Teensy 4
 *   with an added electret mic or the Line-in inputs (not tested).  Or you can use a standalone I2S
 *   microphone like the INMP441 with the Teensy 3 or Teensy 4, this was used for testing: https://www.aliexpress.com/item/32962426410.html
 * 
 * Not fully tested with Teensy 3, only I2S Microphone tested
 * Not compatible with ESP32

 * Based on SpectrumAnalyzer by Paul Stoffregen included in the Teensy Audio Library
 * Modified by Jason Coon for the SmartMatrix Library
 * * 
 * Tested with Teensyduino 1.53 and the Teensy Audio Library included with Teensyduino
 ...
 * SDCS, MOSI, MISO, SCK, MEMCS - don't use the shield's SD Card slot or onboard memory chip.  If you need to use an SD card use the Teesny 4.1
 * 
...
AudioAnalyzeFFT256      fft;        //xy=467,147
...
void setup()
{
  ...
  // configure the mixer to equally add left & right, you can adjust the gain value if you want
  mixer1.gain(0, 0.5);
  mixer1.gain(1, 0.5);

  // the INMP441 seems to need a much larger gain
  //mixer1.gain(0, 10.0);
  //mixer1.gain(1, 10.0);
}
